...
public class ReceiveWork implements Runnable
{
	private BlockingQueue Inqueue;
	private InputStreamReader inputStreamReader;
	BufferedReader bufferReader; 	//Receive the string from the buffer
	String receiveText; 	//used to store the elements that are in the string separeted by the terminatpr
	String splitArray[]; 	//define the size of readChar[]
	int readCharSize ; 	//define the Terminator of the elements 
	String suffix; 	//store the number of characters that the function Bufferreade.read() has read
	int byteRead;
	
	public ReceiveWork(BlockingQueue q,InputStreamReader red) throws IOException
	{
	...
	public void run() { 
		...
		int flag;		//indicate if the terminator is the last element in the string 
		int i;
		try
		{
			while(true)
			{	
				char readChar[] = new char[readCharSize];	//used to read from the information from the buffer and transform it in a string

				if((byteRead =( bufferReader.read(readChar,0,readCharSize))) != -1) 	//Wait until there is something to read from the buffer 
					{
